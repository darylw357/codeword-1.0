...
def run_tests(exename):
    ...
    tests = [
        ...
        # Test codeword-match mode
        test_codewordmatch_invalid_args,
        test_codewordmatch_nonexistent_file,
        test_codewordmatch_invalid_pattern,
        test_codewordmatch_one_word,
    ]
...
def test_invalid_mode(exename):
    '''Test that the program produces the correct usage message.'''
    test_failure([exename, '--invalid'], 'Invalid mode "--invalid"\n')
...
def test_anagram_multiple_words(exename):
    ...
    t(['try', 'zoo'], [['try'], ['zoo']])
    t(['zoo', 'try'], [['zoo'], ['try']])
    t(['nap', 'bar'], [['Pan', 'nap', 'pan'], ['bar', 'bra']])
    t(['nap', 'fish'], [['Pan', 'nap', 'pan'], []])
...
def test_anagram_alternate_file(exename):
    ...
    # Test using a different dictionary file
    t(['warned'], [['Andrew', 'wander', 'warden', 'warned']])
    t(['beast'], [['Bates', 'abets', 'baste', 'bates', 'beast', 'beats', 'betas']])
...
CODEWORDMATCH_USAGE = 'Usage: codeword.exe --codeword-match DICTFILE WORDS\n'

def test_codewordmatch_invalid_args(exename):
    '''Test giving invalid input to the spellcheck mode'''
    test_failure([exename, '--codeword-match'], CODEWORDMATCH_USAGE)
    test_failure([exename, '--codeword-match', 'dictfile'], CODEWORDMATCH_USAGE)

def test_codewordmatch_nonexistent_file(exename):
    '''Test specifyin a non-existent dictionary file to spellcheck mode'''
    test_failure([exename, '--codeword-match', '..dictfile', '1'],
            'Error: "..dictfile" does not exist.\n')

def test_codewordmatch(exename, words_file, codewords, matches):
    '''Test expected output of anagrams'''
    lines = []
    for codeword, matches_word in zip(codewords, matches):
        matchesstr = ''.join(' "%s"' % a for a in matches_word)
        line = '"%s":%s\n' % (codeword, matchesstr)
        lines.append(line)
    expected = ''.join(lines)
    cmdline = [exename, '--codeword-match', words_file] + list(codewords)
    test_stdout(cmdline, expected)

def test_codewordmatch_invalid_pattern(exename):
    '''Test giving invalid patterns to codeword-match mode'''
    wfile = join(TEST_FILES_DIR, 'words.txt')
    def t(patterns, bad):
        expected = 'Error: Invalid pattern "%s"\n' % bad
        cmdline = [exename, '--codeword-match', wfile] + patterns
        test_failure(cmdline, expected)

    # Error should be the first invalid pattern.
    t(['asd'], 'asd')
    t(['1.2'], '1.2')
    t(['1,2', 'bad'], 'bad')
    t(['1,3000'], '1,3000')
    t(['1,27'], '1,27')
    t(['1,2', 'bad', 'really_bad'], 'bad')


def test_codewordmatch_one_word(exename):
    '''Test giving a single word to codeword-match mode'''
    wfile = join(TEST_FILES_DIR, 'words.txt')
    t = lambda w, a: test_codewordmatch(exename, wfile, w, a)

    # Test with 1 word
    t(['1,2,3,2,3,2'], [['banana', 'rococo']])
    t(['11,12,13,12,13,12'], [['banana', 'rococo']])
    t(['1,2,2'], [["add", "all", "ass", "baa", "bee", "boo", "brr", "coo",
        "ebb", "egg", "ell", "err", "fee", "gee", "goo", "ill", "inn", "lee",
        "moo", "odd", "off", "pee", "see", "tee", "too", "wee", "woo",
        "zoo"]])
